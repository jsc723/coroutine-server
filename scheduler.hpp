...
public:
    ...
    void emplace_coro(handle_t coro, await_state state) {
        switch(state) {
    ...
    struct fd_awaiter {
        ...
        int fd;
        std::unordered_map<int, handle_t> &wait_queue;
        handle_t coro;
...
private:
    ...
    std::deque<handle_t> ready_queue;

    std::unordered_map<int, handle_t> read_wait_queue;
};