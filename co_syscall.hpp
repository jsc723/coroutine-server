...
namespace co_syscall {
    scheduler::task_t accept(scheduler &sche, int __fd, sockaddr *__restrict__ __addr, socklen_t *__restrict__ __addr_len) {
    ...
    scheduler::task_t read(scheduler &sche, int __fd, void *__buf, size_t __nbytes) {
        co_await sche.wait_read(__fd);
        co_return ::read(__fd, __buf, __nbytes);
    }

    //return eithor n or 0 (eof) or -1 (error)
    scheduler::task_t read_exact_n(scheduler &sche, int fd, void *buf, size_t n) {
        size_t total = 0;
        while (total < n) {
    ...
    scheduler::task_t read_package(scheduler &sche, int fd, void *payload) {
        int32_t size;
...
namespace co_rpc {
    ...
    task<int32_t> add_number(scheduler &sche, int fd, int32_t a, int32_t b, int &err) {
